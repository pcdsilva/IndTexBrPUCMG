...
function NormasInternasEdit(props) {
  ...
  useEffect(()=>{
    ...
    setNome(props.location.state.data.nome);
    setCategoria(props.location.state.data.categoria);
    setAnoPublicaçao(props.location.state.data.anoPublicaçao);
    setDataAtualizacao(props.location.state.data.dataAtualizacao);
    setDescricao(props.location.state.data.descricao);
    setLink(props.location.state.data.link);
}, []);
  ...
  function handleLogout(id){
    ...
    async  function hendleEditNormaInterna(e){
        e.preventDefault();
        ...
        try{
            await api.put(`https://wu63epzr79.execute-api.us-east-1.amazonaws.com//GestaoNormas//api//Normas//${id}`, data, {
                headers:{
...
export default NormasInternasEdit;